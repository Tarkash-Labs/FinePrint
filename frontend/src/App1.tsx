...
const COMPARE_EXAMPLES = [
  {
    label: '🎓 Bond: Before vs After',
    type: 'internship',
    v1: `1. Compensation: This is an unpaid position. No stipend will be provided.\n2. Intellectual Property: All software created during the term, including on personal time, is exclusively owned by Initech Solutions.\n3. Bond: If hired full-time, intern must stay 3 years or pay ₹10,00,000 penalty.\n4. Non-Compete: Cannot join any competitor for 18 months after leaving.\n5. Working Hours: Nights and weekends expected during deadlines.`,
    v2: `1. Compensation: Intern will receive a monthly stipend of ₹25,000.\n2. Intellectual Property: Work created during working hours on company equipment is owned by Initech. Personal projects remain property of the intern.\n3. Bond: Penalty clause removed. Standard 30-day notice period applies.\n4. Non-Compete: Non-compete reduced to 6 months, limited to direct product competitors.\n5. Working Hours: Standard 10 AM–7 PM Monday to Friday. No mandatory weekend work.`,
  },
  {
    label: '📝 NDA: Original vs Revised',
    type: 'freelance',
    v1: `1. IP Assignment: All work product, including personal time work, is owned exclusively by Client.\n2. Non-Solicitation: Freelancer may not work with Client's clients or competitors for 24 months.\n3. Payment: Client may withhold payment if deliverables deemed unsatisfactory at Client's sole discretion.\n4. Exclusivity: No other clients permitted during project without written approval.\n5. Termination: Client may terminate with no notice. Freelancer must give 60 days notice.`,
    v2: `1. IP Assignment: Work product created during billed hours is owned by Client. Pre-existing tools and frameworks remain Freelancer's property.\n2. Non-Solicitation: Reduced to 6 months, covering only direct clients Freelancer worked with.\n3. Payment: Payment released within 14 days of milestone approval. Disputes resolved via mutual review.\n4. Exclusivity: Clause removed. Freelancer may take other projects.\n5. Termination: Either party may terminate with 14 days written notice.`,
  },
];
...
function App() {
  ...
  const handleCompare = async () => {
    ...
    try {
      ...
      // Derive risk_delta, score_before, score_after, and severity heatmaps
      // from the individual analysis results included in the /compare response.
      // res1 and res2 are the raw AnalyzeResult objects returned by the backend
      // alongside the comparison summary — zero extra API calls.
      const res1: AnalyzeResult | undefined = data.result_v1;
      const res2: AnalyzeResult | undefined = data.result_v2;

      const scoreBefore = res1?.risk_score ?? null;
      const scoreAfter = res2?.risk_score ?? null;
      const riskDelta = (scoreBefore !== null && scoreAfter !== null) ? scoreAfter - scoreBefore : null;

      // Build severity frequency maps from each result's red_flags
      const countSeverity = (flags: FlaggedClause[]) =>
        flags.reduce((acc, f) => { acc[f.severity] = (acc[f.severity] || 0) + 1; return acc; }, {} as Record<string, number>);

      const severityBefore = res1?.red_flags ? countSeverity(res1.red_flags) : null;
      const severityAfter  = res2?.red_flags ? countSeverity(res2.red_flags)  : null;
      ...
      <main className="app-main container">
        ...
        {!result && !compareResult ? (
                  ...
                  {isCompareMode ? (
                    COMPARE_EXAMPLES.map(ex => (
                      <button
                        ...
                        onClick={() => {
                          setInputMode('text');
                          setContractText(ex.v1);
                          setContractTextV2(ex.v2);
          ...
          <div className="space-y-8 export-root animate-fade-in">
            ...
            {/* Risk Delta Headline */}
            {compareResult.score_before !== null && compareResult.score_after !== null && compareResult.risk_delta !== null && (() => {
              const improved = compareResult.risk_delta < 0;
              const worse    = compareResult.risk_delta > 0;
              const pctChange = compareResult.score_before > 0
                ? Math.abs(Math.round(((compareResult.score_after - compareResult.score_before) / compareResult.score_before) * 100))
                : 0;
              return (
                <div className={`card overflow-hidden relative ${improved ? 'border-emerald-200' : worse ? 'border-red-200' : 'border-slate-200'}`}
                  style={{ background: improved ? 'linear-gradient(135deg,#f0fdf4 0%,#dcfce7 100%)' : worse ? 'linear-gradient(135deg,#fef2f2 0%,#fee2e2 100%)' : 'linear-gradient(135deg,#f8fafc 0%,#f1f5f9 100%)' }}>
                  {/* decorative circle */}
                  <div className="absolute -right-8 -top-8 w-40 h-40 rounded-full opacity-10"
                    style={{ background: improved ? '#16a34a' : worse ? '#dc2626' : '#94a3b8' }} />

                  <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10">
                    {/* left: headline text */}
                    <div className="space-y-1 flex-1">
                      <div className={`text-[10px] font-bold uppercase tracking-[0.15em] ${improved ? 'text-emerald-600' : worse ? 'text-red-600' : 'text-slate-400'}`}>
                        {improved ? '✦ Negotiation Result' : worse ? '⚠ Revision Warning' : '— No Change'}
                      </div>
                      <p className={`text-xl sm:text-2xl font-extrabold leading-snug ${improved ? 'text-emerald-900' : worse ? 'text-red-900' : 'text-slate-700'}`}>
                        {improved
                          ? `Risk dropped from ${compareResult.score_before} → ${compareResult.score_after} after negotiation.`
                          : worse
                            ? `Risk increased from ${compareResult.score_before} → ${compareResult.score_after} in the revision.`
                            : `Risk unchanged at ${compareResult.score_before} across both versions.`}
                      </p>
                      {pctChange > 0 && (
                        <p className={`text-sm font-medium ${improved ? 'text-emerald-700' : 'text-red-700'}`}>
                          {improved ? `${pctChange}% safer` : `${pctChange}% riskier`} than the original.
                        </p>
                      )}
                    </div>

                    {/* right: score comparator */}
                    <div className="flex items-center gap-3 shrink-0">
                      {/* V1 score chip */}
                      <div className="flex flex-col items-center bg-white/70 rounded-2xl px-5 py-3 border border-white shadow-sm">
                        <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1">Original</span>
                        <span className="text-4xl font-extrabold text-slate-700">{compareResult.score_before}</span>
                        <span className="text-[10px] text-slate-400 mt-0.5">/ 100</span>
                      </div>

                      {/* arrow */}
                      <div className={`flex flex-col items-center gap-1`}>
                        <span className={`text-2xl leading-none ${improved ? 'text-emerald-500' : worse ? 'text-red-500' : 'text-slate-300'}`}>
                          {improved ? '↘' : worse ? '↗' : '→'}
                        </span>
                        <span className={`text-xs font-extrabold px-2 py-0.5 rounded-full ${
                          improved ? 'bg-emerald-200 text-emerald-800' : worse ? 'bg-red-200 text-red-800' : 'bg-slate-200 text-slate-600'
                        }`}>
                          {compareResult.risk_delta > 0 ? '+' : ''}{compareResult.risk_delta}
                        </span>
                      </div>

                      {/* V2 score chip */}
                      <div className={`flex flex-col items-center rounded-2xl px-5 py-3 border shadow-sm ${
                        improved ? 'bg-emerald-600 border-emerald-500' : worse ? 'bg-red-600 border-red-500' : 'bg-slate-600 border-slate-500'
                      }`}>
                        <span className="text-[10px] font-bold uppercase tracking-widest text-white/70 mb-1">Revised</span>
                        <span className="text-4xl font-extrabold text-white">{compareResult.score_after}</span>
            ...
            {/* Clause Severity Heatmap */}
            {(compareResult.severity_before || compareResult.severity_after) && (
              <div className="card">
                <div className="flex items-center justify-between mb-5">
                  <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Clause Severity Heatmap</h3>
                  <span className="text-[10px] text-slate-400 font-medium">V1 vs V2 flag counts</span>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {(['high', 'medium', 'low'] as const).map(sev => {
                    const before = compareResult.severity_before?.[sev] ?? 0;
                    const after  = compareResult.severity_after?.[sev]  ?? 0;
                    const delta  = after - before;
                    const maxVal = Math.max(before, after, 1);
                    const sevCfg = {
                      high:   { dot: 'bg-red-500',   barV1: 'bg-red-200',   barV2: 'bg-red-500',   label: 'text-red-700',   chip: 'bg-red-100 text-red-700 border-red-200',   icon: '🔴' },
                      medium: { dot: 'bg-amber-500', barV1: 'bg-amber-200', barV2: 'bg-amber-500', label: 'text-amber-700', chip: 'bg-amber-100 text-amber-700 border-amber-200', icon: '🟡' },
                      low:    { dot: 'bg-sky-500',   barV1: 'bg-sky-200',   barV2: 'bg-sky-500',   label: 'text-sky-700',  chip: 'bg-sky-100 text-sky-700 border-sky-200',   icon: '🔵' },
                    };
                    const c = sevCfg[sev];
                    return (
                      <div key={sev} className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-4">
                        {/* header row */}
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <span className={`w-2.5 h-2.5 rounded-full ${c.dot}`} />
                            <span className={`text-xs font-bold uppercase tracking-widest ${c.label}`}>{sev}</span>
                          </div>
                          {delta !== 0 && (
                            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${c.chip}`}>
                              {delta < 0 ? `▼ ${Math.abs(delta)} fixed` : `▲ ${delta} added`}
                            </span>
                          )}
                          {delta === 0 && before > 0 && (
                            <span className="text-[10px] font-medium text-slate-400 px-2 py-0.5 rounded-full border border-slate-200 bg-white">unchanged</span>
                          )}
                        </div>
                        ...
                        <div className="space-y-1">
                          <div className="flex justify-between items-center">
                            <span className="text-[11px] font-semibold text-slate-500">Original (V1)</span>
                            <span className="text-sm font-extrabold text-slate-700">{before}</span>
                          </div>
                          <div className="h-3 bg-slate-200 rounded-full overflow-hidden">
                            <div className={`h-full rounded-full ${c.barV1}`}
                              style={{ width: `${(before / maxVal) * 100}%`, transition: 'width 0.6s ease' }} />
                          </div>
                        </div>
                        ...
                        <div className="space-y-1">
                          <div className="flex justify-between items-center">
                            <span className="text-[11px] font-semibold text-slate-500">Revised (V2)</span>
                            <span className="text-sm font-extrabold text-slate-700">{after}</span>
                          </div>
                          <div className="h-3 bg-slate-200 rounded-full overflow-hidden">
                            <div className={`h-full rounded-full ${c.barV2}`}
                              style={{ width: `${(after / maxVal) * 100}%`, transition: 'width 0.6s ease' }} />
                          </div>
                        </div>

                        {/* bottom summary line */}
                        <div className={`text-[11px] font-medium pt-1 border-t border-slate-200 ${
                          delta < 0 ? 'text-emerald-600' : delta > 0 ? 'text-red-600' : 'text-slate-400'
                        }`}>
                          {delta < 0
                            ? `${Math.abs(delta)} of ${before} ${sev}-severity clause${before !== 1 ? 's' : ''} resolved.`
                            : delta > 0
                              ? `${delta} new ${sev}-severity clause${delta !== 1 ? 's' : ''} introduced.`
                              : before === 0 ? `No ${sev}-severity clauses in either version.` : `${before} ${sev}-severity clause${before !== 1 ? 's' : ''} still present.`
                          }
                        </div>
                      </div>
                    );
